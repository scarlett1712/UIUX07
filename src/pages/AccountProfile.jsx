...
export default function AccountProfile({ role, triggerToast, patients, setPatients }) {
  ...
  const isPatientFieldModified = (field, currentVal) => {
    if (field === 'dob') {
      return originalPatientData.dob !== convertYMDToDMY(currentVal);
    }
  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        defaultProfile = JSON.parse(cached);
      } catch (e) {
        // Fallback
      }
    }
  ...
  const handleSaveProfile = () => {
    ...
    localStorage.setItem('profile_' + role, JSON.stringify(updated));
    setIsEditing(false);
    setIsSaved(true);
    if (triggerToast) {
      triggerToast('Đã lưu thay đổi thông tin tài khoản thành công!', 'success');
  ...
  const bmi = patientHeight > 0 ? (patientWeight / ((patientHeight / 100) * (patientHeight / 100))).toFixed(1) : '0';

  // --- RENDER UNIFIED ACCOUNT SETTINGS FOR ALL ROLES ---
  return (
    <div className="profile-container animate-fade-in">
      {/* Left side info card */}
      <div className="profile-sidebar">
        <div className="profile-large-avatar" style={{ border: role === 'patient' ? '2px solid #4f46e5' : '2px solid var(--primary-light)', backgroundColor: role === 'patient' ? '#e0e7ff' : undefined }}>
          {role === 'expert' ? (
        ...
        <div className="profile-username">
          {role === 'patient' ? patientName : profile.name}
        </div>
        ...
        {role === 'expert' ? (
          ...
          <div className="profile-stats-mini">
            <div className="profile-stat-item-mini">
              <span className="profile-stat-number">{patientBlood}</span>
              <span className="profile-stat-label">Nhóm máu</span>
            </div>
            <div style={{ width: '1px', backgroundColor: 'var(--border-color)' }} />
            <div className="profile-stat-item-mini">
              <span className="profile-stat-number">{patientHeight}</span>
              <span className="profile-stat-label">Cao (cm)</span>
            </div>
            <div style={{ width: '1px', backgroundColor: 'var(--border-color)' }} />
            <div className="profile-stat-item-mini">
              <span className="profile-stat-number">{patientWeight}</span>
              <span className="profile-stat-label">Nặng (kg)</span>
            </div>
          </div>
        ) : (
          <div className="profile-stats-mini">
      ...
      <div className="flex flex-col gap-4" style={{ height: '100%', overflowY: 'auto', paddingRight: '4px' }}>
        {role === 'patient' ? (
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '16px', height: '100%', minHeight: '0' }}>
            {/* COLUMN 1: Personal Info and Medical Notes */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div className="card" style={{ margin: 0 }}>
                ...
                <div className="form-inputs-container">
                  ...
                  <div className="form-group">
                    <span className="form-group-label">Vai trò</span>
              ...
              {/* GHI CHÚ Y TẾ (BỆNH NHÂN) */}
              <div className="card" style={{ margin: 0 }}>
                <h3 style={{ fontSize: '1.25rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px', marginBottom: '10px' }}>
            ...
            {/* COLUMN 2: Health Indicators, Tip Card & Action Buttons */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', justifyContent: 'space-between', height: '100%' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div className="card" style={{ margin: 0 }}>
                  ...
                  <div className="form-inputs-container">
                    <div className="form-group">
                      <span className="form-group-label">Nhóm máu</span>
                      {isEditing ? (
                        <select
                          value={patientBlood}
                    ...
                    <div className="form-group">
                      <span className="form-group-label">Chiều cao (cm)</span>
                      {isEditing ? (
                        <input
                          type="number"
                          value={patientHeight}
                    ...
                    <div className="form-group">
                      <span className="form-group-label">Cân nặng (kg)</span>
                      {isEditing ? (
                        <input
                          type="number"
                          value={patientWeight}
                    ...
                    <div className="form-group">
                      <span className="form-group-label">BMI</span>
              ...
              {/* ACTION BUTTONS FOR PATIENT (aligned at the bottom of COLUMN 2 to align with bottom of notes card in COLUMN 1) */}
